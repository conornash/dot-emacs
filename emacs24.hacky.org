...
* Project Tint Assignments
#+BEGIN_SRC emacs-lisp :tangle yes
  (defvar conor/tint-assignments
    '()
    "Alist of (PROJECT-NAME . TINT-NAME). Edit here to persist.")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ;; -*- lexical-binding: t; -*-
  (require 'color)
  ;; Per-project background tint for visual identification in Exposé.
  ...
  (defun conor/tint--palette-hex (tint-name)
    ...
    (let ((entry (assoc tint-name conor/tint-palette)))
      (when entry
        (let ((params (cdr entry)))
          (conor/tint--hex (nth 0 params) (nth 1 params))))))

  (defun conor/tint--apply (frame tint-name)
    "Apply TINT-NAME to FRAME."
    (let ((hex (conor/tint--palette-hex tint-name)))
      (when hex
        (set-face-background 'default hex frame)
  ...
  (defun conor/select-project-tint ()
    ...
    (let* ((frame (selected-frame))
      ...
      (let ((existing (assoc project conor/tint-assignments)))
        (if existing
            (setcdr existing chosen)
          (push (cons project chosen) conor/tint-assignments)))
      (conor/tint--apply frame chosen)
      (message "%s → %s  (add to conor/tint-assignments to persist)" project chosen)))

  ;; Apply tint when nameframe creates a new project frame
