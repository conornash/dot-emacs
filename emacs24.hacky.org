...
#+BEGIN_SRC emacs-lisp :tangle yes
  (require 'color)
  ;; Per-project visual identity: coloured border and optional font override.
  ;; 17-colour palette keyed by hue. Border colour derived at high
  ;; saturation/lightness for visibility.
  ...
  (defvar conor/tint-palette
    ...
    "Palette of distinguishable colours.
  Each entry is (NAME . (HUE LIGHTNESS)).")

  (defvar conor/tint-border-width 6
    "Internal border width for project frames.")
  ...
  (defun conor/tint--apply (frame tint-name)
    "Apply border and font for TINT-NAME to FRAME."
    (when-let* ((bdr (conor/tint--border-hex tint-name)))
  ...
  (defun conor/tint-apply-to-frame (&optional frame)
    "Apply the assigned identity to FRAME if a mapping exists."
  ...
  (defun conor/select-project-tint ()
    "Interactively select a colour for the current frame's project."
    (interactive)
    (let* ((frame (selected-frame))
           (project (frame-parameter frame 'name))
           (chosen (completing-read
                    (format "Colour for %s: " project)
  ...
  ;; Apply to all existing frames on eval
  (dolist (f (frame-list))
