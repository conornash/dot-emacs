...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;; Never for remote dirs: `locate-dominating-file' over TRAMP is one
  ;; network round-trip per ancestor (see remote kill-switches below).
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package dired
    ...
    (defun iterm-here (&optional arg)
      "Open current directory at point with shell command 'open -a iTerm'."
      (interactive "P")
      (if arg
          (let ((file (dired-get-file-for-visit)))
            (if (file-exists-p file)
                (dired-smart-shell-command (s-concat "open -a iTerm " file)))))
      (dired-shell-command "open -a iTerm ."))
    (global-set-key (kbd "s-O") 'iterm-here) 
  ...
  (defun dired-quicklook ()
    "Preview file at point using QuickLook (qlmanage) on macOS or sushi on Linux."
    (interactive)
    (let ((file (dired-get-file-for-visit)))
      (cond
       ((eq system-type 'darwin)
        (start-process "qlmanage" nil "qlmanage" "-p" file))
       ((eq system-type 'gnu/linux)
        (start-process "sushi" nil "sushi" file)))))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package gfm-mode
    ...
    :init
    ;; Set globally so it applies even when poly-markdown-mode
    ;; hijacks .md files and activates markdown-mode instead of gfm-mode.
    (setq markdown-command '("pandoc" "--from=gfm+subscript+superscript+smart+hard_line_breaks" "--wrap=preserve" "--standalone" "--variable=margin-left:5%" "--variable=margin-right:5%" "--variable=maxwidth:none")))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package projectile
    ...
    :bind (:map projectile-mode-map
                ...
                ("s-f" . nameframe-switch-frame)
                ("C-c p" . projectile-command-map))
    :config
    (setq projectile-per-project-compilation-buffer t
          projectile-file-exists-remote-cache-expire nil
          ;; Never track remote projects (see the projectile/TRAMP
          ;; kill-switch further down).
          projectile-ignored-project-function #'file-remote-p
          projectile-switch-project-action 'projectile-dired)
    ;; Purge any previously-tracked remote projects so
    ;; switch-project/cleanup never run TRAMP operations on them.
    ;; NOTE: `projectile-mode' does NOT load the saved list from disk;
    ;; that happens lazily via `projectile-load-known-projects'.  We
    ;; must load it explicitly here, otherwise `projectile-known-projects'
    ;; is still nil at :config time and `projectile-save-known-projects'
    ;; would clobber projectile-bookmarks.eld with an empty list
    ;; (data-loss bug introduced in 4d60da4, fixed here).
    (projectile-load-known-projects)
    (setq projectile-known-projects
          (seq-remove #'file-remote-p projectile-known-projects))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (add-hook 'conf-toml-mode-hook #'turn-on-smartparens-strict-mode)
  (add-hook 'conf-toml-mode-hook
            (lambda () (add-hook 'after-save-hook #'delete-trailing-whitespace nil t)))
 #+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq auto-revert-remote-files nil
        ...
        forge-bug-reference-remote-files nil
        ...
        tramp-use-connection-share 'suppress
        tramp-use-scp-direct-remote-copying t
        tramp-verbose 2
        vc-handled-backends '(Git)
        )

  ;; Suppress SSH warnings (host key, post-quantum KEX) inside Tramp.
  ;; OpenSSH 10.x emits multi-line ** WARNING banners that confuse
  ;; Tramp's prompt parser and cause connections to hang.  LogLevel
  ;; ERROR silences them for Tramp only; console SSH is unaffected.
  ;;
  ;; ConnectTimeout bounds the *initial* connect: a dead or unreachable
  ;; host fails fast (10s) instead of blocking Emacs on the default TCP
  ;; timeout.  ServerAlive* let ssh notice a dropped link mid-session and
  ;; exit, which transitions the Tramp process to a dead status that
  ;; `my-tramp-reap-dead-connections' below can then actually reap.
  ...
  ;; Guard for mode changes on remote buffers (late, depth 90):
  ;; disable minor modes AFTER global modes in
  ;; `after-change-major-mode-hook' re-enable them.  Mode changes run
  ;; `kill-all-local-variables' which clears buffer-local minor mode
  ;; states; the `find-file-hook' guards don't re-run so features
  ;; like flycheck, yasnippet, and auto-revert get re-enabled on the
  ;; remote buffer.

  (defun my-guard-remote-mode-change ()
    "Re-apply remote file guards after mode changes."
    (when (file-remote-p default-directory)
      (when (bound-and-true-p flycheck-mode) (flycheck-mode -1))
      ;; flymake backends (e.g. `python-flymake') spawn remote
      ;; processes over TRAMP on every check — same class of problem
      ;; as remote eglot/flycheck.
  ...
  (add-hook 'after-change-major-mode-hook #'my-guard-remote-mode-change 90)

  ;; Remote kill-switches for eglot and project.el (2026-06-12): TRAMP
  ;; can't sustain LSP traffic, and project detection walks remote
  ;; ancestors (network round-trips per level).  Decision: neither
  ;; eglot nor project.el ever touches a remote file.
  ;;
  ;; `eglot-ensure' is the only entry point used in this config
  ;; (emacs24.python.org's python hook); guard interactive `eglot' too.
  (defun my-eglot-never-remote (orig &rest args)
    "Never start eglot in a remote buffer."
    (if (file-remote-p default-directory)
        (when (eq this-command 'eglot)
          (message "eglot disabled for remote files (TRAMP)"))
      (apply orig args)))

  (advice-add 'eglot-ensure :around #'my-eglot-never-remote)
  (advice-add 'eglot :around #'my-eglot-never-remote)

  ;; `project-current' is the chokepoint for all of project.el (and
  ;; everything built on it: eglot, project-find-file, vc integration
  ;; in some modes).  Returning nil = "no project" for remote dirs;
  ;; callers degrade gracefully.  This replaces the old
  ;; `memoize-project-current'/`my-populate-remote-caches' machinery,
  ;; which only cached results per exact directory string — every NEW
  ;; remote subdir still paid for a full `project-try-vc' ancestor
  ;; walk.
  (defun my-project-never-remote (orig &optional may-prompt directory)
    "Return nil (no project) for remote directories."
    (let ((dir (or directory
                   project-current-directory-override
                   default-directory)))
      (unless (file-remote-p dir)
        (funcall orig may-prompt directory))))

  (advice-add 'project-current :around #'my-project-never-remote)

  ;; Dired over TRAMP: two chronic slow paths, measured live 2026-06-12
  ;; against patAvailabilityVM (cold subdir visit was ~5-6s, now ~1s).
  ;;
  ;; 1. Projectile/TRAMP kill-switch: `projectile-project-root' walks
  ;;    every ancestor of a remote dir checking ~50 marker files; with
  ;;    `tramp-stat-file-attributes' + `test -e' per level that's 2
  ;;    network round-trips per ancestor (~5s measured for a depth-4
  ;;    path), and it's called from find-file/dired hooks and the
  ;;    mode-line.  Decision: projectile must NEVER interact with
  ;;    remote files.  Returning nil here makes projectile treat every
  ;;    remote buffer as "not in a project", which short-circuits all
  ;;    downstream features (tracking, caching, mode-line, commands).
  ;;    Combined with `projectile-ignored-project-function'
  ;;    #'file-remote-p and purging remote `projectile-known-projects'
  ;;    entries (see use-package block above).  project.el is equally
  ;;    disabled for remote dirs (`my-project-never-remote' above), so
  ;;    remote buffers have NO project awareness at all, by design.
  (defun my-projectile-never-remote (orig &optional dir)
    "Return nil (no project) for any remote DIR, bypassing ORIG entirely."
  ...
  (with-eval-after-load 'projectile
    (advice-add 'projectile-project-root :around
                #'my-projectile-never-remote))

  ;; 2. `dired-insert-directory' calls `dired--insert-disk-space' →
  ;;    `get-free-disk-space' → remote `df' on EVERY readin, a full
  ;;    round-trip per dired buffer (~0.5s each).  Note: setting
  ;;    `dired-free-space' to nil buffer-locally does NOT prevent the
  ;;    df call — `dired--insert-disk-space' calls
  ;;    `get-free-disk-space' before consulting `dired-free-space' for
  ;;    everything except deleting the "total" line.  So advise the df
  ;;    call itself for remote dirs.
  (defun my-no-free-disk-space-for-remote (orig dir)
    "Skip the `df' round-trip for remote directories."
    (unless (file-remote-p dir) (funcall orig dir)))

  (advice-add 'get-free-disk-space :around
              #'my-no-free-disk-space-for-remote)
  ...
  (defun memoize-vc-git-root (orig file)
    (let ((value (memoize-remote (file-name-directory file) 'vc-git-root-cache orig file)))
      ;; Remove nil entries for LOCAL paths only (allows re-check after
      ;; git init).  Keep nil entries for remote paths to avoid repeated
      ;; TRAMP operations on every call.
      (when (and (null (cdr (car vc-git-root-cache)))
                 (not (file-remote-p (file-name-directory file))))
  ...
  ;; Reap zombie TRAMP connections: when SSH dies, clean up TRAMP's
  ;; internal state so the next access reconnects instead of hanging.
  (defun my-tramp-reap-dead-connections ()
    "Kill TRAMP processes whose underlying process has exited.
Scheduled on a timer so stale connections are reaped automatically.

The connection VECTOR lives on the process plist under `tramp-vector'
(this is how `tramp-process-sentinel' itself retrieves it); the old
`tramp-get-connection-property' lookup returned nil, so the cleanup
branch never ran and stale connection state was left behind.  Use
`process-live-p' rather than an explicit status list so half-open
connections in any non-live state are caught."
    (dolist (proc (process-list))
      (when (and (string-match-p "\\*tramp/" (process-name proc))
                 (not (process-live-p proc)))
        (let ((vec (process-get proc 'tramp-vector)))
          (when vec
            (tramp-message vec 2 "Reaping dead TRAMP process: %s" (process-name proc))
            (ignore-errors
              (tramp-cleanup-connection vec 'keep-debug 'keep-password))))
        (ignore-errors (delete-process proc)))))

  (run-with-timer 30 30 #'my-tramp-reap-dead-connections)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package yaml-mode
    :mode "\\.lookml\\'" "\\.yaml.template\\'" "\\.yaml\\'" "\\.yml\\'" "\\.lobster\\'")
...
#+begin_src emacs-lisp :tangle yes
  (use-package graphviz-dot-mode
    :mode "\\.dot\\'"
    :straight (:host github :repo "ppareit/graphviz-dot-mode")
    :config
    (setq graphviz-dot-view-command "neato %s"
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package tide
    ...
    ;; Lever 2: tide passes these straight to tsserver. Tabs, not spaces.
    (setq tide-format-options '(:convertTabsToSpaces nil :tabSize 4 :indentSize 4)))
  ...
  (use-package typescript-mode
    ...
    (typescript-mode . (lambda ()
                         ...
                         ;; Lever 1: override the global `indent-tabs-mode nil`
                         ;; for TypeScript buffers only (both vars are
...
#+begin_src emacs-lisp :tangle yes
  (use-package mermaid-mode
    :mode "\\.mermaid\\'"
    :straight (:host github :repo "abrochard/mermaid-mode")
    :config
...
** Calc Script (ecalc)
#+BEGIN_SRC emacs-lisp :tangle yes
(org-babel-load-if-exists (expand-file-name "emacs29.calc-script.org" user-emacs-directory))
#+END_SRC
...
** Project Layout
Reset the current frame to a standard 4-pane project layout.  All
buffer lookups are strictly scoped by the projectile project root
directory, so buffers from other projects (e.g. pi-coding-agent
sessions for project A) are never shown in project B's frame.

Layout:
#+begin_example
┌──────────────┬──────────────┐
│                        │                        │
│   pi chat              │   Working buffer       │
│                        │                        │
│                        ├──────────────┤
├──────────────┤                        │
│   pi input (~10 lines) │   magit-status         │
├──────────────┴──────────────┤
│   ghostel *ghostel-<project>* (~12 lines)       │
└─────────────────────────────┘
#+end_example

Degrades gracefully: if no pi session exists, the left column is
omitted.  If not a git repo, the magit pane is omitted.  If ghostel
is unavailable, the terminal strip is omitted.

The ghostel buffer is found purely by name — =*ghostel-<project>*=
from the projectile project name, uniquify-style — so re-running the
layout after the frame gets modified picks up the same terminal.  No
project.el involvement.

#+begin_src emacs-lisp :tangle yes
  (defun conor/project-layout ()
    "Reset the current frame to a standard project layout.
  Left column: pi-coding-agent chat (top) + input (bottom).
  Right column: working buffer (top) + magit-status (bottom).
  Bottom strip: project-named ghostel terminal spanning the frame.
  All buffer lookups are scoped to the projectile project root,
  so buffers from other projects are never shown.
  If no pi session exists, shows working buffer full-left.
  If not a git repo, skips the magit pane.
  If ghostel is unavailable, skips the terminal."
    (interactive)
    (let* ((proj-dir (or (projectile-project-root) default-directory))
           (default-directory proj-dir)
           ;; 1. pi-coding-agent buffers (strict directory match)
           (chat-buf (pi-coding-agent--find-session proj-dir))
           (input-buf (when chat-buf
                        (buffer-local-value 'pi-coding-agent--input-buffer
                                            chat-buf)))
           ;; 2. magit-status buffer (scoped by magit-toplevel from default-directory)
           (magit-buf (when (magit-toplevel)
                        (or (magit-get-mode-buffer 'magit-status-mode)
                            (progn (magit-status-setup-buffer proj-dir)
                                   (magit-get-mode-buffer 'magit-status-mode)))))
           ;; 3. Working buffer: most recent file-visiting project buffer
           ;;    excluding agent, magit, helm, and hidden buffers
           (work-buf (or (car (seq-filter
                           (lambda (b)
                             (let ((name (buffer-name b)))
                               (and (not (string-match-p "pi-coding-agent" name))
                                    (not (string-match-p "magit" name))
                                    (not (string-prefix-p " " name))
                                    (not (string-match-p "helm" name))
                                    (buffer-file-name b))))
                           (projectile-project-buffers)))
                         (dired-noselect proj-dir)))
           ;; 4. Ghostel terminal, named *ghostel-<project>* from the
           ;;    projectile project name.  Pure buffer naming (uniquify
           ;;    style), no project.el: reuse a live buffer with that
           ;;    name, else let-bind `ghostel-buffer-name' so `ghostel'
           ;;    creates it.  save-window-excursion undoes its
           ;;    pop-to-buffer.
           (ghostel-buf (when (fboundp 'ghostel)
                          (let ((ghostel-buffer-name
                                 (format "*ghostel-%s*"
                                         (projectile-project-name proj-dir))))
                            (or (get-buffer ghostel-buffer-name)
                                (ignore-errors
                                  (save-window-excursion (ghostel)))))))
           (has-pi (and chat-buf input-buf))
           (has-magit (not (null magit-buf))))
      (delete-other-windows)
      ;; Bottom terminal strip spans the full frame; the column layout
      ;; below operates on the remaining top window.
      (when ghostel-buf
        (let ((term-win (split-window (selected-window) -12 'below)))
          (set-window-buffer term-win ghostel-buf)
          (set-window-dedicated-p term-win 'soft)))
      (cond
       ;; Full 4-pane: pi left, work+magit right
       ((and has-pi has-magit)
        (let* ((left-win (selected-window))
               (right-win (split-window left-win nil 'right))
               (input-win (split-window left-win -10 'below))
               (magit-win (split-window right-win
                            (floor (* 0.6 (window-total-height right-win)))
                            'below)))
          (set-window-buffer left-win chat-buf)
          (set-window-buffer input-win input-buf)
          (set-window-buffer right-win work-buf)
          (set-window-buffer magit-win magit-buf)
          (set-window-dedicated-p left-win 'soft)
          (set-window-dedicated-p input-win 'soft)
          (set-window-dedicated-p magit-win 'soft)
          (select-window input-win)))
       ;; Pi but no magit: pi left, work right
       (has-pi
        (let* ((left-win (selected-window))
               (right-win (split-window left-win nil 'right))
               (input-win (split-window left-win -10 'below)))
          ...
          (set-window-dedicated-p input-win 'soft)
          (select-window input-win)))
       ;; No pi, has magit: work left, magit right
       (has-magit
        (let* ((left-win (selected-window))
               (right-win (split-window left-win nil 'right)))
          (set-window-buffer left-win work-buf)
          (set-window-buffer right-win magit-buf)
          (set-window-dedicated-p right-win 'soft)
          (select-window left-win)))
       ;; Nothing: just the working buffer
       (t
...
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package ytr
  :straight (:host github :repo "xenodium/ytr")
  :commands (ytr)
  :config
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (global-set-key (kbd "<f9>") 'conor/project-layout)
  (global-set-key (kbd "<C-M-f9>") 'conor/project-layout)
#+END_SRC
...
The default read-process-output-max is 64KB, which is still quite conservative. Modern LSP servers like rust-analyzer or clangd routinely send multi-megabyte responses. Bumping this reduces the number of read calls Emacs has to make: [[*Eglot][Eglot]]
