...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (add-to-list 'package-archives '("org" . "http://orgmode.org/elpa/")) ;; Goes down often
...
* Language Server Protocol (LSP) / Flycheck
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package lsp-mode
    ...
    (setq
     ...
     lsp-pyls-server-command '("pipenv" "run" "pyls")
     lsp-restart 'ignore)
    (lsp-register-client
     (make-lsp-client :new-connection (lsp-tramp-connection '("pipenv" "run" "pyls"))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (add-hook 'json-mode-hook
            #'(lambda ()
                ...
                (define-key json-mode-map (kbd "<M-tab>") 'origami-recursively-toggle-node)
                (define-key json-mode-map (kbd "<S-tab>") 'origami-toggle-all-nodes)
                (define-key json-mode-map (kbd "C-M-n") 'origami-next-fold)
                (define-key json-mode-map (kbd "C-M-p") 'origami-previous-fold)
                (define-key json-mode-map (kbd "C-M-b") 'origami-backward-fold-same-level)
                (define-key json-mode-map (kbd "C-M-f") 'origami-forward-fold-same-level)
                ))
 #+END_SRC
