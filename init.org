...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;; Two-phase guard for mode changes on remote buffers.
  ;; Phase 1 (early, depth -90): populate caches BEFORE eglot's
  ;; `eglot--maybe-activate-editing-mode' calls `project-current',
  ;; which otherwise triggers projectile to walk the remote directory
  ;; tree — dozens of network-requiring TRAMP operations that hang
  ;; with a slow or broken SSH connection.
  ;; Phase 2 (late, depth 90): disable minor modes AFTER global modes
  ;; in `after-change-major-mode-hook' re-enable them.  Mode changes
  ;; run `kill-all-local-variables' which clears buffer-local minor
  ;; mode states; the `find-file-hook' guards don't re-run so
  ;; features like flycheck, yasnippet, and auto-revert get
  ;; re-enabled on the remote buffer.

  (defun my-populate-remote-caches ()
    "Pre-populate project caches for remote buffers before eglot runs."
    (when (file-remote-p default-directory)
      (unless (assoc default-directory project-current-cache)
        (push (cons default-directory nil) project-current-cache))))
  ...
  (add-hook 'after-change-major-mode-hook #'my-populate-remote-caches -90)
